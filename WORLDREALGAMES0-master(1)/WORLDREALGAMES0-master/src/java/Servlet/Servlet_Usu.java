...
public class Servlet_Usu extends HttpServlet {
            ...
            if (usuario.equals(usu.getIdUsuario())) {
                if (clave.equals(usu.getClaveUsuario())) {
                    if ("HAB".equals(usu.getEstadoUsuario())) {
                        ...
                        encodedURL = response.encodeRedirectURL("Catalogo.jsp");
                        response.sendRedirect(encodedURL);
                    } else {
                        response.sendRedirect("Login.jsp?mens='Usted ha sido inhabilitado del sistema");
                    }
                } else {
                    response.sendRedirect("Login.jsp?mens='Su clave es incorrecto'");
                }
            } else {
                response.sendRedirect("Login.jsp?mens='Su id de usuario es incorrecto'");
            }
  ...
  //      usu.setDniUsuario(Dni);
    ...
    //    usu.setDistritoUsuario(Distrito);
        ...
        if (resp) {
            response.sendRedirect("RegistrarCliente.jsp?mens='Se ha registrado correctamente'");
        } else {
            response.sendRedirect("RegistrarCliente.jsp?mens='Error al registrarse'");
        }
    }
    ...
    //    usu.setDistritoUsuario(request.getParameter("txtDistrito"));
        ...
        if (resp) {
            response.sendRedirect("Catalogo.jsp?mens='Sus datos se han modificado correctamente'");
        } else {
            response.sendRedirect("mensaje2_1.jsp?mens='No se pudo modificar uno de sus datos'");
        }
    ...
    private void DarBajaUsuario(HttpServletRequest request, HttpServletResponse response)
        ...
        if (resp) {
            response.sendRedirect("MostrarUsuario.jsp?mens='El usuario " + usu.getCodigoUsuario() + " ha sido inhabilitado correctamente'");
        } else {
            response.sendRedirect("MostrarUsuario.jsp?mens='No se pudo eliminar el usuario'");
        }
    }
    ...
    private void DarAltaUsuario(HttpServletRequest request, HttpServletResponse response)
        ...
        if (resp) {
            response.sendRedirect("MostrarUsuarioEliminado.jsp?mens='El usuario " + usu.getCodigoUsuario() + " acaba de ser habilitado satisfactoriamente'");
        } else {
            response.sendRedirect("MostrarUsuarioEliminado.jsp?mens='No se pudo recuperar el usuario'");
        }
    }
