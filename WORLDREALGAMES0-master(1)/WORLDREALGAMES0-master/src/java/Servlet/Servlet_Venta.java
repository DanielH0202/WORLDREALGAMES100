...
public class Servlet_Venta extends HttpServlet {
        ...
        if(resp){
            ...
            for(int i=0; i<NombreProducto.length;i++){
                try{
                    ...
                    if(j==1){
                        response.sendRedirect("RegistrarVenta.jsp?mens='Se ha registrado su compra correctamente, lo contactaremos para confirmar la compra. '"); 
                        lista.clear();
                    }else{
                        response.sendRedirect("mensaje2.jsp?mens='No se pudo registrar su compra'");
                    }
                }catch(Exception e){System.out.println(e);}
            }
        }else{
            response.sendRedirect("RegistrarVenta.jsp?mens='No se pudo registrar su compra, intentelo nuevamente.'");
        }
    }
    ...
    private void QuitarProducto(HttpServletRequest request, HttpServletResponse response)
        ...
        response.sendRedirect("Catalogo1.jsp"); 
    }
    ...
    private void EliminarVenta(HttpServletRequest request, HttpServletResponse response)
        ...
        if(resp){
            response.sendRedirect("MostrarVenta.jsp?mens='Erro al eliminar la venta "+ v.getCodigoVenta() +"'"); 
        }else{
            response.sendRedirect("MostrarVenta.jsp?mens='Se ha eliminado la venta correctamente " +v.getCodigoVenta()+ "'"); 
        }
    }
